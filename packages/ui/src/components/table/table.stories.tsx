...
function makeDate(x: number): Order[] {
  ...
  const getRandomId = () => `order_${Math.floor(Math.random() * 100000)}`;

  const getRandomDisplayId = () => Math.floor(Math.random() * 100000);

  const getRandomAmount = () => Math.floor(Math.random() * 1000);

  const getRandomCurrency = () => currencies[Math.floor(Math.random() * currencies.length)];

  const getRandomEmail = () => `${Math.floor(Math.random() * 100000)}@gmail.com`;
