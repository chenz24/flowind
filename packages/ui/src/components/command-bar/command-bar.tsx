...
const Value = React.forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<'div'>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={clx('txt-compact-small-plus text-ui-contrast-fg-secondary px-3 py-2.5', className)}
      {...props}
    />
  ),
);
...
>(({ className, ...props }, ref) => (
  <div ref={ref} className={clx('bg-ui-contrast-border-base h-10 w-px', className)} {...props} />
));
