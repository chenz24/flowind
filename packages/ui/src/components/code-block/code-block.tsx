...
const Root = ({
  ...
  return (
    <CodeBlockContext.Provider value={{ snippets, active, setActive }}>
      <div
        className={clx(
          'border-code-border flex flex-col overflow-hidden rounded-lg border',
          className,
        )}
        {...props}
      >
...
const HeaderComponent = ({
  ...
  return (
    <div
      className={clx(
        'border-b-code-border bg-code-bg-subtle flex items-center gap-2 border-b px-4 py-3',
        className,
      )}
      {...props}
    >
        ...
        snippets.map((snippet) => (
          <div
            className={clx(
              ...
              {
                'text-code-fg-base border-code-border bg-code-bg-base cursor-default':
...
const Meta = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={clx('txt-compact-sm text-code-fg-subtle ml-auto', className)} {...props} />
);
...
const Body = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
  ...
  return (
    <div
      className={clx('bg-code-bg-base relative h-full overflow-y-auto p-4', className)}
      {...props}
    >
      {!active.hideCopy && (
        <Copy content={active.code} className="text-code-fg-muted absolute right-4 top-4" />
      )}
            ...
            >
              {!active.hideLineNumbers && (
                <div role="presentation" className="flex flex-col text-right">
                  {tokens.map((_, i) => (
                    <span key={i} className="text-code-fg-subtle tabular-nums">
                      {i + 1}
                    </span>
                  ))}
                </div>
