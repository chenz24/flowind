...
import { ChevronDown } from '@flowind/icons';
import { clx, cva, DefaultProps, Selectors, useComponentDefaultProps } from '@/styles';
import { createPolymorphicComponent } from '@/utils/create-polymorphic-component';
import useStyles from './navigation-menu.styles';

export type NavMenuStylesNames = Selectors<typeof useStyles>;
export interface NavigationMenuProps extends DefaultProps<NavMenuStylesNames>, NavProps {}

const defaultProps: Partial<NavigationMenuProps> = {};

const _NavigationMenu = forwardRef<HTMLDivElement, NavigationMenuProps>((props, ref) => {
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.List
    ref={ref}
    className={clx('group flex flex-1 list-none items-center justify-center space-x-1', className)}
    {...props}
  />
));
...
const navigationMenuTriggerStyle = cva({
  base: `group inline-flex h-10 w-max items-center justify-center rounded-md bg-background px-4 py-2 text-sm font-medium 
...
>(({ className, children, ...props }, ref) => (
  <NavigationMenuPrimitive.Trigger
    ref={ref}
    className={clx(navigationMenuTriggerStyle(), 'group', className)}
    {...props}
  >
    {children}{' '}
    <ChevronDown
...
>(({ className, ...props }, ref) => (
  <div className={clx('absolute left-0 top-full flex justify-center')}>
...
_NavigationMenu.List = NavigationMenuList;
_NavigationMenu.Item = NavigationMenuItem;
_NavigationMenu.Trigger = NavigationMenuTrigger;
_NavigationMenu.triggerStyle = navigationMenuTriggerStyle;
_NavigationMenu.Link = NavigationMenuLink;
_NavigationMenu.Content = NavigationMenuContent;
_NavigationMenu.Viewport = NavigationMenuViewport;
_NavigationMenu.Indicator = NavigationMenuIndicator;

export const NavigationMenu = createPolymorphicComponent<
  'NavigationMenu',
  NavigationMenuProps,
  {
    ...
    triggerStyle: typeof navigationMenuTriggerStyle;
  }
>(_NavigationMenu);
